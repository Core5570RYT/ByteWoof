...
class TitleState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			{				
				FlxG.switchState(new WarningScreen());
			});
	...
	override function beatHit()
	{
		...
		{
			case 1:
				createCoolText(['core', 'and myself']);
			...
			// credTextShit.text += '\nNight';
			case 15:
				addMoreText('the wolf'); // credTextShit.text += '\nFunkin';
	...
	function titleMovements():Void //this code is trash
		{
			//new FlxTimer().start(0.3, function (tmr: FlxTimer) { //creating a timer
			//for byte
			FlxTween.tween(bytetitle, {x: FlxG.width * 0.59, y: FlxG.height * 0.07}, 1.0,
				{ease: FlxEase.smoothStepOut, onComplete: function(twn: FlxTween){

				}});

			//for logo
			FlxTween.tween(logoBl, {x: -150, y: -100}, 1.0,
				{ease: FlxEase.smoothStepOut, onComplete: function(twn: FlxTween){
