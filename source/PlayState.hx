...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'outside':
				{
					...
					var bg:FlxSprite = new FlxSprite(-250,-150).loadGraphic(Paths.image('philly/sky', 'week3'));
					bg.setGraphicSize(Std.int(bg.width * 1.1));
					bg.scrollFactor.set(0.3, 0.3);
					add(bg);

					var city:FlxSprite = new FlxSprite(-300,-150).loadGraphic(Paths.image('philly/city', 'week3'));
					city.setGraphicSize(Std.int(city.width * 1.1));
					city.scrollFactor.set(0.5, 0.5);
					city.updateHitbox();
					add(city);

					outsideCityLights = new FlxTypedGroup<FlxSprite>();
					if(FlxG.save.data.distractions){
						add(outsideCityLights);
					}
					...
					for (i in 0...5)
					{
							...
							light.updateHitbox();
							light.antialiasing = true;
							outsideCityLights.add(light);
					}

					var street:FlxSprite = new FlxSprite(-200, -50).loadGraphic(Paths.image('philly/street','week3'));
		...
		oof.finishThing = startCountdown;

		Conductor.songPosition = -5000;
		
		strumLine = new FlxSprite(30, 50).makeGraphic(FlxG.width, 10);
		...
		if (SONG.song.toLowerCase() == 'byte')
			{
				...
				FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));
				remove(black);
				dad.playAnim('byteCutscene', true);
				dad.animation.pause();
			}
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					if (SONG.song.toLowerCase() == 'byte')
					{	
						//camera.focusOn(dad.getGraphicMidpoint());
						dad.playAnim('byteCutscene', true);
						new FlxTimer().start(1.8, function (woofWoof:FlxTimer) {
	...
	override public function update(elapsed:Float)
	{
		...
		for (i in 0...strumLineNotes.length)
			{
				...
				strumLineNotes.members[i].x = (FlxMath.lerp(80 + (i * strumLineNotes.members[i].width), strumLineNotes.members[i].x, 0.95));	
			}
		...
		for (i in 0...playerStrums.length)
			{
				...
				playerStrums.members[i].x = (FlxMath.lerp(900 + (i * playerStrums.members[i].width), playerStrums.members[i].x, 0.95));	
			}
		iconP1.updateHitbox();
		iconP2.updateHitbox();
		...
		if (camZooming)
		{
			...
			camHUD.x = FlxMath.lerp(0, camHUD.x, 0.95);
		...
		if (health <= 0)
		{
			...
			openSubState(new GameOverSubstate(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y, gf.getScreenPosition().x, gf.getScreenPosition().y));
	...
	function schoolOutro(?dialogueBox:DialogueBox):Void
		{
			...
			inCutscene = true;
			add(dialogueBox);
			dialogueBox.finishThing = woofNewSongSwitch;
		}

	function woofNewSongSwitch():Void
		{
			LoadingState.loadAndSwitchState(new PlayState());
		}
	...
	override function beatHit()
	{
			...
			if (curSong.toLowerCase() == 'byte' && curBeat >= 52 && curBeat < 56 && camZooming && FlxG.camera.zoom < 2.35)
				{
					FlxG.camera.zoom += 0.020;
					camHUD.zoom += 0.04;
					if (curStage == 'bytestage')
						{
							cyanneh.alpha = 0.5;
							FlxTween.tween(cyanneh, {alpha: 0}, 1.0,
		...
		if (curSong.toLowerCase() == 'howl' && curBeat >= 96 && curBeat < 128 && camZooming && FlxG.camera.zoom < 1.35)
			{
				...
				if (curStage == 'bytestage')
					{
					whiteeh.color = FlxColor.BLACK;
					cyanneh.alpha = 0.3;
					cyanneh.color = FlxColor.BLUE;
					FlxTween.tween(cyanneh, {alpha: 0}, 0.15,
					{ease: FlxEase.quadOut , onComplete: function(twn: FlxTween){
					}});
				}
			}
		...
		if (!inCutscene)
			...
			if (camZooming && FlxG.camera.zoom < 1.35 && curBeat % 4 == 0)
			{
				...
				if (curStage == 'bytestage')
					{
						cyanneh.alpha = 0.5;
						FlxTween.tween(cyanneh, {alpha: 0}, 0.6,
							{ease: FlxEase.quadOut , onComplete: function(twn: FlxTween){
							}});
					}
			}
		...
		switch (curStage)
		{
			...
			case "outside":
				if (curBeat % 2 == 0)
					{
						outsideCityLights.forEach(function(light:FlxSprite)
						{
							light.visible = false;
						});
	
						curLight = FlxG.random.int(0, outsideCityLights.length - 1);
	
						outsideCityLights.members[curLight].visible = true;
				}
		}
	...
	function healthBarMove()
		{
			if (FlxG.save.data.downscroll)
				{
					FlxTween.tween(iconP1, {y: iconP1.y - 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});
					FlxTween.tween(iconP2, {y: iconP2.y - 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});
					FlxTween.tween(healthBar, {y: healthBar.y - 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});
					FlxTween.tween(healthBarBG, {y: healthBarBG.y - 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});
					FlxTween.tween(scoreTxt, {y: scoreTxt.y - 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});				
				} else{
					FlxTween.tween(iconP1, {y: iconP1.y + 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});
					FlxTween.tween(iconP2, {y: iconP2.y + 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});
					FlxTween.tween(healthBar, {y: healthBar.y + 150}, 2.0, {ease: FlxEase.circOut, onComplete:function(twn:FlxTween){}});
